...
/*
    ...
    creating Bayesian Networks.  It allows the user to graphically draw the
   network, save/load the network to/from disk, and also to calculate the
   posterior probability of any node in the network given a set of evidence.
    ...
    If you want to understand how to use the Bayesian Network utilities in the
   library you should definitely look at the bayes_net_ex.cpp example program.
   It gives a comprehensive introduction to creating and manipulating Bayesian
   Networks.  If you want to see how to load a saved network from disk and use
   it in a non-GUI application then look at the bayes_net_from_disk_ex.cpp
   example.


    Now all of that being said, if you have already looked at the other relevant
    examples and want to see a more in-depth example then by all means, continue
   reading. :)
*/
...
typedef graph<dlib::set<unsigned long>::compare_1b_c,
              dlib::set<unsigned long>::compare_1b_c>::kernel_1a_c
    join_tree_type;
...
class main_window : public drawable_window {
  /*!
      ...
      WHAT THIS OBJECT REPRESENTS
          This object is the main window of a utility for drawing Bayesian
  Networks. It allows you to draw a directed graph and to set the conditional
  probability tables up for each node in the network.  It also allows you to
  compute the posterior probability of each node.  And finally, it lets you save
  and load networks from file
  !*/
public:
  main_window();
  ~main_window();
...
private:
  ...
  // The std_vector_c is an object identical to the std::vector except that it
  // checks all its preconditions and throws a dlib::fatal_error if they are
  // violated.
...
#ifdef WIN32
//  If you use main() as your entry point when building a program on MS Windows
//  then there will be a black console window associated with your application.
//  If you want your application to not have this console window then you need
//  to build using the WinMain() entry point as shown below and also set your
//  compiler to produce a "Windows" project instead of a "Console" project.  In
//  visual studio this can be accomplished by going to
//  project->properties->general configuration-> Linker->System->SubSystem and
//  selecting Windows instead of Console.
//
...
main_window::main_window()
    : color_non_evidence(0, 0, 0), color_default_bg(255, 255, 255),
      color_evidence(100, 200, 100), color_error(255, 0, 0),
      color_gray(210, 210, 210), graph_modified_since_last_recalc(true),
      btn_calculate(*this), sel_node_is_evidence(*this), graph_drawer(*this),
      sel_node_index(*this), sel_node_num_values_label(*this),
      sel_node_text_label(*this), sel_node_evidence_label(*this), mbar(*this),
      selected_node_rect(*this), tables(*this), sel_node_num_values(*this),
      sel_node_text(*this), sel_node_evidence(*this), cpt_grid(*this),
      ppt_grid(*this), selected_node_index(0), node_is_selected(false),
      cpt_group(*this), ppt_group(*this) {
  // Note that all the GUI widgets take a reference to the window that contains
  // them as their constructor argument.  This is a universal feature of GUI
  // widgets in the dlib library.

  set_title("Bayesian Network Utility");

  // position the widget that is responsible for drawing the directed graph, the
  // graph_drawer, just below the mbar (menu bar) widget.
  graph_drawer.set_pos(5, mbar.bottom() + 5);
  set_size(750, 400);

  // register the event handlers with their respective widgets
  btn_calculate.set_click_handler(*this,
                                  &main_window::recalculate_probabilities);
  cpt_grid.set_text_modified_handler(*this, &main_window::on_cpt_grid_modified);
  graph_drawer.set_graph_modified_handler(*this,
                                          &main_window::on_graph_modified);
  graph_drawer.set_node_deleted_handler(*this, &main_window::on_node_deleted);
  graph_drawer.set_node_deselected_handler(*this,
                                           &main_window::on_node_deselected);
  graph_drawer.set_node_selected_handler(*this, &main_window::on_node_selected);
  sel_node_evidence.set_text_modified_handler(
      *this, &main_window::on_sel_node_evidence_modified);
  sel_node_is_evidence.set_click_handler(*this,
                                         &main_window::on_evidence_toggled);
  sel_node_num_values.set_text_modified_handler(
      *this, &main_window::on_sel_node_num_values_modified);
  sel_node_text.set_text_modified_handler(
      *this, &main_window::on_sel_node_text_modified);
  ...
  // Now setup the tabbed display.  It will have two tabs, one for the
  // conditional probability table and one for the posterior probability table.
  ...
  mbar.menu(0).add_menu_item(
      menu_item_text("Open", *this, &main_window::on_menu_file_open, 'O'));
  mbar.menu(0).add_menu_item(menu_item_separator());
  mbar.menu(0).add_menu_item(
      menu_item_text("Save", *this, &main_window::on_menu_file_save, 'S'));
  mbar.menu(0).add_menu_item(menu_item_text(
      "Save As", *this, &main_window::on_menu_file_save_as, 'a'));
  mbar.menu(0).add_menu_item(menu_item_separator());
  mbar.menu(0).add_menu_item(
      menu_item_text("Quit", *this, &main_window::on_menu_file_quit, 'Q'));

  // Add the entries to the Help menu.
  mbar.menu(1).add_menu_item(
      menu_item_text("Help", *this, &main_window::on_menu_help_help, 'e'));
  mbar.menu(1).add_menu_item(
      menu_item_text("About", *this, &main_window::on_menu_help_about, 'A'));
...
void main_window::load_selected_node_tables_into_ppt_grid() {
  ...
  // load the top row of the table into the grid.  This row is the "title bar"
  // row that tells you what each column contains.
...
void main_window::load_selected_node_tables_into_cpt_grid() {
  ...
  // fill in the top row of the grid that shows which parent node the left hand
  // columns go with
  assignment a(
      node_first_parent_assignment(graph_drawer.graph(), selected_node_index));
  unsigned long col = 0;
  a.reset();
  while (a.move_next()) {
    cpt_grid.set_text(0, col,
                      cast_to_string(a.element().key()) + ": " +
                          graph_drawer.node_label(a.element().key()));
    cpt_grid.set_background_color(0, col, rgb_pixel(120, 210, 210));
    cpt_grid.set_editable(0, col, false);
    ++col;
  }

  // fill in the top row of the grid that shows which probability the right hand
  // columns go with
  for (col = node.number_of_parents(); col < num_cols; ++col) {
    cpt_grid.set_text(
        0, col,
        "P(node=" + cast_to_string(col - node.number_of_parents()) + ")");
  ...
  do {
    ...
    // fill in the left side of the grid row that shows what the parent
    // assignment is
    a.reset();
    while (a.move_next()) {
    ...
    // fill in the right side of the grid row that shows what the conditional
    // probabilities are
    ...
    ++row;
  } while (node_next_parent_assignment(graph_drawer.graph(),
                                       selected_node_index, a));
}

// ----------------------------------------------------------------------------------------

void main_window::initialize_node_cpt_if_necessary(unsigned long index) {
  ...
  if (node_cpt_filled_out(graph_drawer.graph(), index) == false) {
    ...
    // loop over all the possible parent assignments for this node and fill them
    // out with reasonable default values
...
void main_window::no_node_selected() {
  // Make it so that no node is selected on the gui.  Do this by disabling
  // things and clearing out text fields and so forth.
...
void main_window::recalculate_probabilities() {
  // clear out the current solution
  solution.reset();
  if (graph_is_connected(graph_drawer.graph()) == false) {
    message_box("Error", "Your graph has nodes that are completely "
    ...
    solution.reset(
        new bayesian_network_join_tree(graph_drawer.graph(), join_tree));
...
void main_window::on_open_file_selected(const std::string &file_name) {
  try {
    no_node_selected();
...
void main_window::on_save_file_selected(const std::string &file_name) {
...
void main_window::on_menu_file_save_as() {
  // display a file chooser window and when the user choses a file
  // call the on_save_file_selected() function
  save_file_box(*this, &main_window::on_save_file_selected);
}
...
void main_window::on_menu_file_quit() { close_window(); }

// ----------------------------------------------------------------------------------------

// This event is called when the user selects from the menu bar Help->Help
void main_window::on_menu_help_help() {
  message_box("Help", "To create new nodes right click on the drawing area.\n"
                      "To create edges select the parent node and then "
                      "shift+left click on the child node.\n"
                      "To remove nodes or edges select them by left clicking "
...
void main_window::on_menu_help_about() {
  message_box(
      "About",
      "This application is the GUI front end to the dlib C++ Library's\n"
...
// This event is called when the user modifies the graph_drawer widget.  That
// is, when the user adds or removes an edge or node in the graph.
void main_window::on_graph_modified() {
...
void main_window::on_sel_node_evidence_modified() {
...
// a node.
void main_window::on_sel_node_num_values_modified() {
  // make a reference to the node in the graph that is currently selected
  node_type &node = graph_drawer.graph_node(selected_node_index);
...
// This event is called when the user modifies the cpt_grid (i.e. the
// conditional probability table widget)
void main_window::on_cpt_grid_modified(unsigned long row, unsigned long col) {
  node_type &node = graph_drawer.graph_node(selected_node_index);
  solution.reset();
  ...
  node.data.table().set_probability(cur_val, cpt_grid_assignments[row - 1],
                                    prob);
  ...
  if (sum > 1.0) {
    cpt_grid.set_background_color(row, cpt_grid.number_of_columns() - 1,
                                  color_error);
  } else {
    // edit one of the other elements in the table to ensure that the
    // probabilities still sum to 1
    if (cur_val == node.data.table().num_values() - 1) {
      node.data.table().set_probability(0, cpt_grid_assignments[row - 1],
                                        1 - sum);
      cpt_grid.set_text(row, node.number_of_parents(), cast_to_string(1 - sum));
    } else {
      node.data.table().set_probability(node.data.table().num_values() - 1,
                                        cpt_grid_assignments[row - 1], 1 - sum);
      cpt_grid.set_text(row, cpt_grid.number_of_columns() - 1,
                        cast_to_string(1 - sum));
    }
...
// This event is called when the user resizes the main_window.  Note that unlike
// the other events, this event is part of the drawable_window base class that
// main_window inherits from. So you won't see any statements in the constructor
// that say "register the main_window::on_window_resized function"
void main_window::on_window_resized() {
  // when you override any of the drawable_window events you have to make sure
  // you call the drawable_window's version of them because it needs to process
  ...
  // Don't do anything if the user just made the window too small.  That is,
  // leave the widgets where they are.
  ...
  // Now position all the widgets in the window.  Note that much of the
  // positioning is relative to other widgets.  This part of the code I just
  // figured out by trying stuff and rerunning the program to see if it looked
  // nice.
  sel_node_index.set_pos(graph_drawer.right() + 14, graph_drawer.top() + 18);
  sel_node_text_label.set_pos(sel_node_index.left(),
                              sel_node_index.bottom() + 5);
  sel_node_text.set_pos(sel_node_text_label.right() + 5,
                        sel_node_index.bottom());
  sel_node_num_values_label.set_pos(sel_node_index.left(),
                                    sel_node_text.bottom() + 5);
  sel_node_num_values.set_pos(sel_node_num_values_label.right(),
                              sel_node_text.bottom() + 5);
  sel_node_is_evidence.set_pos(sel_node_index.left(),
                               sel_node_num_values.bottom() + 5);
  sel_node_evidence_label.set_pos(sel_node_index.left(),
                                  sel_node_is_evidence.bottom() + 5);
  sel_node_evidence.set_pos(sel_node_evidence_label.right() + 5,
                            sel_node_is_evidence.bottom());
  ...
  sel_node_num_values.set_width(tables.right() - sel_node_num_values.left() +
                                1);

  // Tell the named rectangle to position itself such that it fits around the
  // tabbed display that contains the probability tables and the label at the
  // top of the screen.
  selected_node_rect.wrap_around(sel_node_index.get_rect() + tables.get_rect());

  // finally set the button to be at the bottom of the named rectangle
  btn_calculate.set_pos(selected_node_rect.left(),
                        selected_node_rect.bottom() + 5);
}

// ----------------------------------------------------------------------------------------

// This event is called by the graph_drawer widget when the user selects a node
void main_window::on_node_selected(unsigned long n) {
  ...
  if (graph_drawer.graph_node(n).data.is_evidence()) {
    ...
    sel_node_evidence.set_text(
        cast_to_string(graph_drawer.graph_node(n).data.value()));
  } else {
    sel_node_is_evidence.set_unchecked();
    sel_node_evidence.disable();
    sel_node_evidence.set_text("");
  ...
  sel_node_num_values.set_text(
      cast_to_string(node_num_values(graph_drawer.graph(), n)));
...
void main_window::on_evidence_toggled() {
  if (sel_node_is_evidence.is_checked()) {
    graph_drawer.graph_node(selected_node_index).data.set_as_evidence();
    sel_node_evidence.enable();
    sel_node_evidence.set_text(cast_to_string(
        graph_drawer.graph_node(selected_node_index).data.value()));
...
void main_window::on_node_deselected(unsigned long) { no_node_selected(); }

// ----------------------------------------------------------------------------------------

// This event is called when the user causes a node to be deleted
void main_window::on_node_deleted() { no_node_selected(); }

// ----------------------------------------------------------------------------------------

// This event is called when the user changes the text in the "node label" text
// field
void main_window::on_sel_node_text_modified() {
