...
Int_t mb_button_id[13] = {kMBYes,    kMBNo,     kMBOk,     kMBApply,  kMBRetry,
                          kMBIgnore, kMBCancel, kMBClose,  kMBYesAll, kMBNoAll,
                          kMBNewer,  kMBAppend, kMBDismiss};

EMsgBoxIcon mb_icon[4] = {kMBIconStop, kMBIconQuestion, kMBIconExclamation,
                          kMBIconAsterisk};

const char *filetypes[] = {
    "All files", "*",          "ROOT files",     "*.root", "ROOT macros",
...
shutterData_t function_data[] = {{"f1_s.xpm", "TF1", 2001, nullptr},
...
const char *editortxt1 =
    ...
    "be used to edit scripts or to present users editable config files, "
    "etc.\n\n"
...
const char *editortxt2 = "Mice with scroll-ball are properly supported.\n\n"
                         ...
                         "text cursor and moves the\n"
                         "    cursor one position to the left. If a text has "
                         "been marked by the user";
const char *editortxt3 =
    "    (e.g. by clicking and dragging) the cursor will be put at the "
    "beginning\n"
    "    of the marked text and the marked text will be removed.\n"
    "Home\n"
    "    Moves the text cursor to the left end of the line. If mark is TRUE "
    "text\n"
    "    will be marked towards the first position, if not any marked text "
    "will\n"
    "    be unmarked if the cursor is moved.\n"
    "End\n"
    "    Moves the text cursor to the right end of the line. If mark is TRUE "
    "text\n"
    "    will be marked towards the last position, if not any marked text "
    "will\n"
    "    be unmarked if the cursor is moved.\n"
    "Delete";
const char *editortxt4 = "    Deletes the character on the right side of the "
                         "text cursor. If a text\n"
                         "    has been marked by the user (e.g. by clicking "
                         "and dragging) the cursor\n"
                         "    will be put at the beginning of the marked text "
                         "and the marked text will\n"
                         "    be removed.\n"
                         "Shift - Left Arrow\n"
...
TestMainFrame::TestMainFrame(const TGWindow *p, UInt_t w, UInt_t h) {
  ...
  fMenuDock->Connect("Undocked()", "TestMainFrame", this,
  ...
  for (int i = 0; i < 256; ++i)
    fCanvasWindow->AddFrame(new TGFrame(fCanvasWindow->GetContainer(), 32, 32,
                                        0,
  ...
  fStatusFrame =
      new TGCompositeFrame(fMain, 60, 20, kHorizontalFrame | kSunkenFrame);
  ...
  fStatusFrame->AddFrame(
      fTestButton, new TGLayoutHints(kLHintsTop | kLHintsLeft, 2, 0, 2, 2));
  ...
  fStatusFrame->AddFrame(
      fTestText, new TGLayoutHints(kLHintsTop | kLHintsLeft, 10, 2, 2, 2));
  Pixel_t yellow;
  gClient->GetColorByName("yellow", yellow);
  fColorSel = new TGColorSelect(fStatusFrame, yellow, 0);
  fStatusFrame->AddFrame(
      fColorSel, new TGLayoutHints(kLHintsTop | kLHintsLeft, 2, 0, 2, 2));

  fMain->AddFrame(
      fStatusFrame,
      new TGLayoutHints(kLHintsBottom | kLHintsExpandX, 0, 0, 1, 0));
...
void TestMainFrame::HandleMenu(Int_t id) {
  ...
  case M_TEST_FONTDIALOG: {
    ...
    if (prop.fName != "")
      printf("Selected font: %s, size %d, italic %s, bold %s, color 0x%lx, "
             "align %u\n",
  ...
  case M_TEST_NEWMENU: {
    ...
    fMenuTest->AddEntry("Remove New Menus", M_NEW_REMOVEMENU, nullptr, nullptr,
                        e);
  } break;
...
TestDialog::TestDialog(const TGWindow *p, const TGWindow *main, UInt_t w,
  ...
  fL1 = new TGLayoutHints(kLHintsTop | kLHintsLeft | kLHintsExpandX, 200, 2, 2,
                          2);
  ...
  fL4 = new TGLayoutHints(
      kLHintsTop | kLHintsLeft | kLHintsExpandX | kLHintsExpandY, 5, 5, 5, 5);
  ...
  fEc1->GetCanvas()->Connect(
      "ProcessedEvent(Int_t,Int_t,Int_t,TObject*)", "TestDialog", this,
  ...
  fF4->AddFrame(fCheckMulti = new TGCheckButton(fF4, "&Mutli Selectable", 92),
                fL3);
...
void TestDialog::DoTab(Int_t id) { printf("Tab item %d activated\n", id); }
...
TestMsgBox::TestMsgBox(const TGWindow *p, const TGWindow *main, UInt_t w,
                       UInt_t h, UInt_t options)
  ...
  fL2 =
      new TGLayoutHints(kLHintsTop | kLHintsRight | kLHintsExpandX, 2, 5, 0, 2);
...
void TestMsgBox::DoTest() {
  ...
  new TGMsgBox(gClient->GetRoot(), fMain, fTbtitle->GetString(),
               fTbmsg->GetString(), icontype, buttons, &retval);
  fMain->Disconnect("CloseWindow()");
  fMain->Connect("CloseWindow()", "TestMsgBox", this, "CloseWindow()");
}
...
TestSliders::TestSliders(const TGWindow *p, const TGWindow *main, UInt_t w,
                         UInt_t h) {
  ...
  fHslider1->Connect("PositionChanged(Int_t)", "TestSliders", this,
                     "DoSlider(Int_t)");
  fHslider1->SetRange(0, 50);

  fVslider1 = new TGVSlider(fVframe1, 100, kSlider2 | kScaleBoth, VSId1);
  fVslider1->Connect("PositionChanged(Int_t)", "TestSliders", this,
                     "DoSlider(Int_t)");
  ...
  fHslider2->Connect("PositionChanged(Int_t)", "TestSliders", this,
                     "DoSlider(Int_t)");
  ...
  fMain->SetWMSizeHints(size.fWidth, size.fHeight, size.fWidth, size.fHeight, 0,
                        0);
...
TestShutter::TestShutter(const TGWindow *p, const TGWindow *main, UInt_t w,
                         UInt_t h) {
...
void TestShutter::CloseWindow() { delete this; }
...
TestDirList::TestDirList(const TGWindow *p, const TGWindow *main, UInt_t w,
                         UInt_t h) {
  ...
  fContents->Connect("DoubleClicked(TGListTreeItem*,Int_t)", "TestDirList",
...
void TestDirList::CloseWindow() { delete this; }
...
TestFileList::TestFileList(const TGWindow *p, const TGWindow *main, UInt_t w,
                           UInt_t h) {
...
void TestFileList::CloseWindow() { delete this; }

TestProgress::TestProgress(const TGWindow *p, const TGWindow *main, UInt_t w,
                           UInt_t h) {
...
EntryTestDlg::EntryTestDlg(const TGWindow *p, const TGWindow *main) {
  ...
  TGFont *myfont =
      gClient->GetFont("-adobe-helvetica-bold-r-*-*-12-*-*-*-*-*-iso8859-1");
  if (myfont)
...
void EntryTestDlg::CloseWindow() { delete this; }
...
void Editor::DoOpen() { SetTitle(); }

void Editor::DoSave() { SetTitle(); }
