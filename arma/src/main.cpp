...
public:
  Window(std::valarray<double> &x, std::valarray<double> &y,
      ...
      : plot_data1(x, y, color), plot(plot_data1, x_title, y_title, plot_title),
        canvas(plot), label1("Plot 1"), label2("Plot 2"),
        ...
        linewidth_spin2(linewidth_adj2, 0.1, 1.0), print_button("Print"),
    ...
    plot.add_data(*Gtk::manage(new Gtk::PLplot::PlotData2D(
        x_va, y_va, Gdk::RGBA("blue"),
        Gtk::PLplot::LineStyle::LONG_DASH_LONG_GAP, 5.0)));
    plot.hide_legend();

    // add some lines
    // plot.add_object(*Gtk::manage(new Gtk::PLplot::PlotObject2DLine(-5, -1,
    // 30, -2)));
    plot.add_object(
        *Gtk::manage(new Gtk::PLplot::PlotObject2DLine(30, -2, -5, -1)));
    plot.add_object(*Gtk::manage(
        new Gtk::PLplot::PlotObject2DLine(Gtk::Orientation::HORIZONTAL, -1)));
    plot.add_object(*Gtk::manage(
        new Gtk::PLplot::PlotObject2DLine(Gtk::Orientation::VERTICAL, 12)));
    ...
    Gtk::AspectFrame geometry(Gtk::Align::CENTER, Gtk::Align::CENTER,
    ...
    color_combo1.signal_color_set().connect(
        [this]() { plot_data1.set_color(color_combo1.get_rgba()); });
    color_combo2.signal_color_set().connect([this, plot_data2]() {
      plot_data2->set_color(color_combo2.get_rgba());
    });

    linestyle_combo1.signal_changed().connect([this]() {
      plot_data1.set_line_style(static_cast<Gtk::PLplot::LineStyle>(
          linestyle_combo1.get_active_row_number() + 1));
    });
    linestyle_combo2.signal_changed().connect([this, plot_data2]() {
      plot_data2->set_line_style(static_cast<Gtk::PLplot::LineStyle>(
          linestyle_combo2.get_active_row_number() + 1));
    });
    ...
    linewidth_spin1.signal_value_changed().connect(
        [this]() { plot_data1.set_line_width(linewidth_spin1.get_value()); });
    linewidth_spin2.signal_value_changed().connect([this, plot_data2]() {
      plot_data2->set_line_width(linewidth_spin2.get_value());
    });
    ...
    print_button.signal_clicked().connect(
        sigc::mem_fun(*this, &Window::on_print_button_clicked));
    saveas_button.signal_clicked().connect(
        sigc::mem_fun(*this, &Window::on_saveas_button_clicked));
  ...
  void on_print_button_clicked() {
    ...
    page_setup->set_paper_size_and_default_margins(
        Gtk::PaperSize(Gtk::PAPER_NAME_A4));
    ...
    operation->signal_draw_page().connect(
        sigc::mem_fun(*this, &Window::on_draw_page));
    operation->set_n_pages(1);

    if (Gtk::PrintOperation::Result::APPLY !=
        operation->run(Gtk::PrintOperation::Action::PRINT_DIALOG, *this)) {
      // error handling
    }
  ...
  void on_saveas_button_clicked() {
    auto dialog =
        new Gtk::FileChooserDialog("Save as", Gtk::FileChooser::Action::SAVE);
  ...
  void on_file_dialog_response(int response_id,
    ...
    if (filter_selected->get_name() == "EPS") {
      if (filename.compare(filename.length() - 4, std::string::npos, ".eps") !=
          0)
        filename += ".eps";

      Cairo::RefPtr<Cairo::PsSurface> surface =
          Cairo::PsSurface::create(filename, 842, 595);
      ...
      if (filename.compare(filename.length() - 4, std::string::npos, ".png") !=
          0)
        filename += ".png";

      Cairo::RefPtr<Cairo::ImageSurface> surface = Cairo::ImageSurface::create(
          Cairo::ImageSurface::Format::ARGB32, 842, 595);
      Cairo::RefPtr<Cairo::Context> cr = Cairo::Context::create(surface);
      ...
      if (filename.compare(filename.length() - 4, std::string::npos, ".pdf") !=
          0)
        filename += ".pdf";

      Cairo::RefPtr<Cairo::PdfSurface> surface =
          Cairo::PdfSurface::create(filename, 842, 595);
      Cairo::RefPtr<Cairo::Context> cr = Cairo::Context::create(surface);
...
int main(int argc, char **argv) {
  ...
  Glib::RefPtr<Gtk::Application> app =
      Gtk::Application::create("eu.tomschoonjans.gtkmm-plplot-test1");
  ...
  return app->make_window_and_run<Test1::Window>(argc, argv, x_va, y_va, "x",
                                                 "y");
}
