...
protected:
...
#else
  void OnRightUp(wxMouseEvent& event) { ShowContextMenu(event.GetPosition()); }
#endif

  void OnMenuOpen(wxMenuEvent& event)
  {
#if USE_LOG_WINDOW
    LogMenuOpenCloseOrHighlight(event, wxT("opened"));
    event.Skip();
#endif
  }
  ...
  {
#if USE_LOG_WINDOW
    LogMenuOpenCloseOrHighlight(event, wxT("closed"));
    event.Skip();
#endif
  }
  ...
  {
#if USE_LOG_WINDOW
    LogMenuOpenCloseOrHighlight(event, wxT("highlighted"));
    event.Skip();
#endif
  }
...
private:
#if USE_LOG_WINDOW
  void LogMenuOpenCloseOrHighlight(const wxMenuEvent& event,
                                   const wxChar* what);
#endif
  ...
  : wxFrame((wxFrame*)NULL, wxID_ANY, wxT("wxWidgets menu sample"))
{
...
#if USE_LOG_WINDOW
  wxMenuItem* item =
    new wxMenuItem(fileMenu, Menu_File_ClearLog, wxT("Clear &log\tCtrl-L"));
  ...
  fileMenu->Append(
    Menu_File_ShowDialog, wxT("Show &Dialog\tCtrl-D"), wxT("Show a dialog"));
  fileMenu->AppendSeparator();

  fileMenu->Append(
    Menu_File_Quit, wxT("E&xit\tAlt-X"), wxT("Quit menu sample"));

  wxMenu* menubarMenu = new wxMenu;
  menubarMenu->Append(Menu_MenuBar_Append,
                      wxT("&Append menu\tCtrl-A"),
                      wxT("Append a menu to the menubar"));
  menubarMenu->Append(Menu_MenuBar_Insert,
                      wxT("&Insert menu\tCtrl-I"),
                      wxT("Insert a menu into the menubar"));
  menubarMenu->Append(Menu_MenuBar_Delete,
                      wxT("&Delete menu\tCtrl-D"),
                      wxT("Delete the last menu from the menubar"));
  menubarMenu->Append(Menu_MenuBar_Toggle,
                      wxT("&Toggle menu\tCtrl-T"),
                      wxT("Toggle the first menu in the menubar"),
                      true);
  menubarMenu->AppendSeparator();
  menubarMenu->Append(Menu_MenuBar_Enable,
                      wxT("&Enable menu\tCtrl-E"),
                      wxT("Enable or disable the last menu"),
                      true);
  menubarMenu->AppendSeparator();
  menubarMenu->Append(Menu_MenuBar_GetLabel,
                      wxT("&Get menu label\tCtrl-G"),
                      wxT("Get the label of the last menu"));
#if wxUSE_TEXTDLG
  menubarMenu->Append(Menu_MenuBar_SetLabel,
                      wxT("&Set menu label\tCtrl-S"),
                      wxT("Change the label of the last menu"));
  menubarMenu->AppendSeparator();
  menubarMenu->Append(Menu_MenuBar_FindMenu,
                      wxT("&Find menu from label\tCtrl-F"),
                      wxT("Find a menu by searching for its label"));
#endif

  wxMenu* subMenu = new wxMenu;
  subMenu->Append(Menu_SubMenu_Normal,
                  wxT("&Normal submenu item"),
                  wxT("Disabled submenu item"));
  subMenu->AppendCheckItem(
    Menu_SubMenu_Check, wxT("&Check submenu item"), wxT("Check submenu item"));
  subMenu->AppendRadioItem(
    Menu_SubMenu_Radio1, wxT("Radio item &1"), wxT("Radio item"));
  subMenu->AppendRadioItem(
    Menu_SubMenu_Radio2, wxT("Radio item &2"), wxT("Radio item"));
  subMenu->AppendRadioItem(
    Menu_SubMenu_Radio3, wxT("Radio item &3"), wxT("Radio item"));

  menubarMenu->Append(Menu_SubMenu, wxT("Submenu"), subMenu);

  wxMenu* menuMenu = new wxMenu;
  menuMenu->Append(Menu_Menu_Append,
                   wxT("&Append menu item\tAlt-A"),
                   wxT("Append a menu item to the 'Test' menu"));
  menuMenu->Append(Menu_Menu_AppendSub,
                   wxT("&Append sub menu\tAlt-S"),
                   wxT("Append a sub menu to the 'Test' menu"));
  menuMenu->Append(Menu_Menu_Insert,
                   wxT("&Insert menu item\tAlt-I"),
                   wxT("Insert a menu item in head of the 'Test' menu"));
  menuMenu->Append(Menu_Menu_Delete,
                   wxT("&Delete menu item\tAlt-D"),
                   wxT("Delete the last menu item from the 'Test' menu"));
  menuMenu->Append(Menu_Menu_DeleteSub,
                   wxT("Delete last &submenu\tAlt-K"),
                   wxT("Delete the last submenu from the 'Test' menu"));
  menuMenu->AppendSeparator();
  menuMenu->Append(Menu_Menu_Enable,
                   wxT("&Enable menu item\tAlt-E"),
                   wxT("Enable or disable the last menu item"),
                   true);
  menuMenu->Append(Menu_Menu_Check,
                   wxT("&Check menu item\tAlt-C"),
                   wxT("Check or uncheck the last menu item"),
                   true);
  menuMenu->AppendSeparator();
  menuMenu->Append(Menu_Menu_GetInfo,
                   wxT("Get menu item in&fo\tAlt-F"),
                   wxT("Show the state of the last menu item"));
#if wxUSE_TEXTDLG
  menuMenu->Append(Menu_Menu_SetLabel,
                   wxT("Set menu item label\tAlt-L"),
                   wxT("Set the label of a menu item"));
#endif
#if wxUSE_TEXTDLG
  menuMenu->AppendSeparator();
  menuMenu->Append(Menu_Menu_FindItem,
  ...
  helpMenu->Append(
    Menu_Help_About, wxT("&About\tF1"), wxT("About menu sample"));
...
wxMenu*
MyFrame::CreateDummyMenu(wxString* title)
{
...
void
MyFrame::LogMenuEvent(const wxCommandEvent& event)
{
  int id = event.GetId();
...
void
MyFrame::OnClearLog(wxCommandEvent& WXUNUSED(event))
{
  m_textctrl->Clear();
}

void
MyFrame::OnClearLogUpdateUI(wxUpdateUIEvent& event)
{
...
void
MyFrame::OnShowDialog(wxCommandEvent& WXUNUSED(event))
{
  MyDialog dlg(this);
  dlg.ShowModal();
}
...
void
MyFrame::OnDeleteMenu(wxCommandEvent& WXUNUSED(event))
{
...
void
MyFrame::OnInsertMenu(wxCommandEvent& WXUNUSED(event))
{
  wxString title;
...
void
MyFrame::OnAppendMenu(wxCommandEvent& WXUNUSED(event))
{
  wxString title;
  wxMenu* menu = CreateDummyMenu(&title);
...
void
MyFrame::OnToggleMenu(wxCommandEvent& WXUNUSED(event))
{
  wxMenuBar* mbar = GetMenuBar();
  if (!m_menu) {
    // hide the menu
...
void
MyFrame::OnEnableMenu(wxCommandEvent& event)
{
  wxMenuBar* mbar = GetMenuBar();
  size_t count = mbar->GetMenuCount();
...
void
MyFrame::OnGetLabelMenu(wxCommandEvent& WXUNUSED(event))
{
  wxMenuBar* mbar = GetMenuBar();
  size_t count = mbar->GetMenuCount();
...
#if wxUSE_TEXTDLG
void
MyFrame::OnSetLabelMenu(wxCommandEvent& WXUNUSED(event))
{
  wxMenuBar* mbar = GetMenuBar();
  size_t count = mbar->GetMenuCount();
  ...
  wxString label = wxGetTextFromUser(wxT("Enter new label: "),
...
void
MyFrame::OnFindMenu(wxCommandEvent& WXUNUSED(event))
{
  wxMenuBar* mbar = GetMenuBar();
  size_t count = mbar->GetMenuCount();
  ...
  wxString label = wxGetTextFromUser(
    wxT("Enter label to search for: "), wxT("Find menu"), wxEmptyString, this);
...
void
MyFrame::OnDummy(wxCommandEvent& event)
{
  wxLogMessage(wxT("Dummy item #%d"), event.GetId() - Menu_Dummy_First + 1);
}

void
MyFrame::OnAppendMenuItem(wxCommandEvent& WXUNUSED(event))
{
  wxMenuBar* menubar = GetMenuBar();
  wxMenu* menu = menubar->GetMenu(menubar->FindMenu("Test"));
...
void
MyFrame::OnAppendSubMenu(wxCommandEvent& WXUNUSED(event))
{
  wxMenuBar* menubar = GetMenuBar();
  wxMenu* menu = menubar->GetMenu(menubar->FindMenu("Test"));
  wxCHECK_RET(menu, wxT("no 'Test' menu?"));

  menu->Append(Menu_Dummy_Last,
               wxT("&Dummy sub menu"),
...
void
MyFrame::OnDeleteMenuItem(wxCommandEvent& WXUNUSED(event))
{
  wxMenuBar* menubar = GetMenuBar();
  wxMenu* menu = menubar->GetMenu(menubar->FindMenu("Test"));
  wxCHECK_RET(menu, wxT("no 'Test' menu?"));
...
void
MyFrame::OnDeleteSubMenu(wxCommandEvent& WXUNUSED(event))
{
  wxMenuBar* menubar = GetMenuBar();
  wxMenu* menu = menubar->GetMenu(menubar->FindMenu("Test"));
  wxCHECK_RET(menu, wxT("no 'Test' menu?"));
...
void
MyFrame::OnInsertMenuItem(wxCommandEvent& WXUNUSED(event))
{
  wxMenuBar* menubar = GetMenuBar();
  wxMenu* menu = menubar->GetMenu(menubar->FindMenu("Test"));
  wxCHECK_RET(menu, wxT("no 'Test' menu?"));
  ...
  menu->Insert(0,
               wxMenuItem::New(
                 menu, Menu_Dummy_Fourth, wxT("Fourth dummy item\tCtrl-F4")));
  menu->Insert(1, wxMenuItem::New(menu, wxID_SEPARATOR));
}

void
MyFrame::OnEnableMenuItem(wxCommandEvent& WXUNUSED(event))
{
...
void
MyFrame::OnCheckMenuItem(wxCommandEvent& WXUNUSED(event))
{
  wxMenuItem* item = GetLastMenuItem();
...
void
MyFrame::OnGetLabelMenuItem(wxCommandEvent& WXUNUSED(event))
{
  wxMenuItem* item = GetLastMenuItem();

  if (item) {
    wxString label = item->GetItemLabel();
    wxLogMessage(wxT("The label of the last menu item is '%s'"), label.c_str());
  }
}

#if wxUSE_TEXTDLG
void
MyFrame::OnSetLabelMenuItem(wxCommandEvent& WXUNUSED(event))
{
  wxMenuItem* item = GetLastMenuItem();
...
void
MyFrame::OnGetMenuItemInfo(wxCommandEvent& WXUNUSED(event))
{
  wxMenuItem* item = GetLastMenuItem();
  ...
  if (item) {
    wxString msg;
    msg << wxT("The item is ")
        << (item->IsEnabled() ? wxT("enabled") : wxT("disabled")) << wxT('\n');

    if (item->IsCheckable()) {
      msg << wxT("It is checkable and ")
          << (item->IsChecked() ? wxT("") : wxT("un")) << wxT("checked\n");
    }
...
#if wxUSE_TEXTDLG
void
MyFrame::OnFindMenuItem(wxCommandEvent& WXUNUSED(event))
{
  wxMenuBar* mbar = GetMenuBar();
  size_t count = mbar->GetMenuCount();
  ...
  wxString label = wxGetTextFromUser(wxT("Enter label to search for: "),
                                     wxT("Find menu item"),
                                     wxEmptyString,
                                     this);
  ...
  if (!label.empty()) {
    ...
    for (menuindex = 0; (menuindex < count) && (index == wxNOT_FOUND);
         ++menuindex) {
...
void
MyFrame::ShowContextMenu(const wxPoint& pos)
{
  wxMenu menu;
  ...
  } else // normal case, shift not pressed
  {
    ...
    menu.Append(Menu_Popup_ToBeGreyed,
                wxT("To be &greyed"),
...
void
MyFrame::OnTestNormal(wxCommandEvent& WXUNUSED(event))
{
  wxLogMessage(wxT("Normal item selected"));
}

void
MyFrame::OnTestCheck(wxCommandEvent& event)
{
...
void
MyFrame::OnTestRadio(wxCommandEvent& event)
{
...
#if USE_LOG_WINDOW
void
MyFrame::LogMenuOpenCloseOrHighlight(const wxMenuEvent& event,
                                     const wxChar* what)
{
  wxString msg;
  msg << wxT("A ") << (event.IsPopup() ? wxT("popup ") : wxT(""))
      << wxT("menu has been ") << what;
...
void
MyFrame::OnUpdateSubMenuCheck(wxUpdateUIEvent& event)
{
  event.Enable(true);
}
  ...
  : wxDialog(parent, wxID_ANY, "Test Dialog")
{
  ...
  m_textctrl->AppendText(
    wxT(
      "Dialogs do not have menus, but popup menus should function the same\n\n")
...
#if USE_LOG_WINDOW
void
MyDialog::LogMenuOpenCloseOrHighlight(const wxMenuEvent& event,
                                      const wxChar* what)
{
  wxString msg;
  msg << wxT("A ") << (event.IsPopup() ? wxT("popup ") : wxT(""))
...
#if USE_CONTEXT_MENU
void
MyDialog::OnContextMenu(wxContextMenuEvent& event)
{
  wxPoint point = event.GetPosition();
  // If from keyboard
  if (point.x == -1 && point.y == -1) {
...
void
MyDialog::ShowContextMenu(const wxPoint& pos)
{
  wxMenu menu;
