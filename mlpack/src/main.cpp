...
int main() {
  ...
  mat dataset;
  Row<size_t> labels;
  if (!data::Load("dataset/covertype-small.data.csv", dataset))
    throw std::runtime_error("Could not read covertype-small.data.csv!");
  if (!data::Load("dataset/covertype-small.labels.csv", labels))
  ...
  RandomForest r(trainDataset, trainLabels, 7 /* number of classes */,
                 10 /* number of trees */, 3 /* minimum leaf size */);
