...
public:
  InfoExampleUI()
      : UI(DISTRHO_UI_DEFAULT_WIDTH, DISTRHO_UI_DEFAULT_HEIGHT),
        fSampleRate(getSampleRate()), fResizable(isResizable()), fScale(1.0f),
        fScaleFactor(getScaleFactor()), fResizeHandle(this) {
    std::memset(fParameters, 0, sizeof(float) * kParameterCount);
    std::memset(fStrBuf, 0, sizeof(char) * (0xff + 1));

#ifdef DGL_NO_SHARED_RESOURCES
    createFontFromFile("sans",
...
protected:
  ...
  void parameterChanged(uint32_t index, float value) override {
    // some hosts send parameter change events for output parameters even when
    // nothing changed we catch that here in order to prevent excessive repaints
  ...
  void onNanoDisplay() override {
    ...
    drawRight(x, y,
              (fParameters[kParameterCanRequestParameterValueChanges] > 0.5f)
                  ? "Yes"
                  : "No",
              40);
    y += lineHeight;
  ...
  void onResize(const ResizeEvent &ev) override {
    fScale = static_cast<float>(ev.size.getHeight()) /
             static_cast<float>(DISTRHO_UI_DEFAULT_HEIGHT);
...
private:
  ...
  void drawLeft(float x, const float y, const char *const text,
                const int offset = 0) {
  ...
  void drawRight(float x, const float y, const char *const text,
                 const int offset = 0) {
    const float width = (100.0f + offset) * fScale;
    x += offset * fScale;
    beginPath();
...
UI *createUI() { return new InfoExampleUI(); }
