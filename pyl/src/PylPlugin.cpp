...
protected:
  ...
  /**
     ...
     0-9 characters.
   */
  const char *getLabel() const override { return "Info"; }
   ...
   */
  int64_t getUniqueId() const override { return d_cconst('d', 'N', 'f', 'o'); }
  ...
  /**
     ...
     The host may call this function from any context, including realtime
     processing. When a parameter is marked as automatable, you must ensure no
  ...
  void run(const float **inputs, float **outputs, uint32_t frames) override {
    /**
       ...
       So here we directly copy inputs over outputs, leaving the audio
       untouched. We need to be careful in case the host re-uses the same buffer
...
Plugin *createPlugin() { return new InfoExamplePlugin(); }
